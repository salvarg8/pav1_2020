...
namespace TpiBugs.Reportes
{
    public partial class FrmReporteCursos : Form
    {
        ...
        {
            string strSql = "select Categorias.id_categoria as Categoria, Cursos.nombre as NombreCurso, Cursos.descripcion as Descripción, Cursos.fecha_vigencia as Vigencia, Categorias.nombre as nombre_categ from Cursos, Categorias where (cursos.id_categoria = Categorias.id_categoria and cursos.fecha_vigencia >= getdate() and Categorias.id_categoria = @Valorcmb and Cursos.borrado = 0) group by Categorias.id_categoria, cursos.nombre, cursos.descripcion, cursos.fecha_vigencia, Categorias.nombre";
